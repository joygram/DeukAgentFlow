...
exports.AgentFlowCommandCenter = void 0;
const fs = require("fs");
const path = require("path");
const child_process_1 = require("child_process");
const vscode = require("vscode");
const agentFlowModel_1 = require("./agentFlowModel");
const agentFlowViewUtils_1 = require("./agentFlowViewUtils");
const agentFlowViewScript_1 = require("./agentFlowViewScript");
const i18n_1 = require("./i18n");
const VIEW_TYPE = 'deukagentflow-command-center';
const PANEL_TYPE = 'deukagentflow-panel';
class AgentFlowCommandCenter {
    constructor(context, model) {
        this.context = context;
        this.model = model;
        this.views = new Map();
        this.panel = null;
        this.skillTemplatesDir = undefined;
    ...
    async openPanel() {
        ...
        this.panel = vscode.window.createWebviewPanel(PANEL_TYPE, 'AgentFlow Panel', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async openTicket(ticketId) {
        ...
        if (!ticket) {
            void vscode.window.showInformationMessage((0, i18n_1.t)('noActiveTicket'));
            return;
        }
    ...
    async copyHandoff() {
        ...
        void vscode.window.showInformationMessage((0, i18n_1.t)('handoffCopied'));
    }
    async sendHandoffToTarget() {
        await this.sendTextToTarget(this.model.getSnapshot().handoff, 'handoff');
    }
    async sendChatToTarget(text) {
    ...
    async createTicketViaAgent() {
        const snapshot = this.model.getSnapshot();
        const targetLabel = (0, agentFlowModel_1.describeTarget)(snapshot.target);
        const prompt = (0, i18n_1.t)('createTicketPrompt');
        const delivery = await this.deliverToTarget(snapshot.target, prompt);
        if (delivery === 'direct') {
            void vscode.window.showInformationMessage((0, i18n_1.t)('createTicketSentDirect').replace('{target}', targetLabel));
            return;
        }
        if (delivery === 'attempted') {
            void vscode.window.showInformationMessage((0, i18n_1.t)('createTicketSentAttempted').replace('{target}', targetLabel));
            return;
        }
        void vscode.window.showInformationMessage((0, i18n_1.t)('createTicketSentClipboard').replace('{target}', targetLabel));
    }
    async sendTextToTarget(text, source) {
        const snapshot = this.model.getSnapshot();
        ...
        if (!payload) {
            void vscode.window.showInformationMessage((0, i18n_1.t)('nothingToSend'));
            return;
        }
    ...
    async runCliCommand(kind) {
        ...
        if (snapshot.command.status === 'running') {
            void vscode.window.showInformationMessage((0, i18n_1.t)('commandAlreadyRunning'));
            return;
        }
    ...
    async runTicketLifecycleAction(action, ticketId) {
        const snapshot = this.model.getSnapshot();
        const ticket = ticketId ? this.model.findTicket(ticketId) : (snapshot.previewTicket ?? snapshot.activeTicket);
        if (!ticket) {
            void vscode.window.showInformationMessage((0, i18n_1.t)('noSelectedTicket'));
            return;
        }
        if (!(0, agentFlowModel_1.ticketLifecycleActions)(ticket).includes(action)) {
        ...
        const confirmed = await vscode.window.showWarningMessage(`${ticket.id} 티켓에 ${confirmLabel} 액션을 실행할까요?`, { modal: action !== 'close' }, confirmLabel);
    ...
    async executeShellCommand(command, cwd, trackOutput = true) {
    ...
    resolveSkillTemplatesDir() {
        ...
        try {
            ...
            const located = (0, child_process_1.execFileSync)(finder, ['deuk-agent-flow'], { encoding: 'utf8' })
                .split(/\r?\n/)
                .map((line) => line.trim())
                .filter(Boolean)[0];
            if (located) {
    ...
    readSkillDoc(skillId) {
        const candidates = [
            ...
            this.resolveSkillTemplatesDir() ? path.join(this.resolveSkillTemplatesDir(), skillId, 'SKILL.md') : '',
        ].filter(Boolean);
        for (const candidate of candidates) {
            try {
                const raw = fs.readFileSync(candidate, 'utf8');
                return (0, agentFlowViewUtils_1.parseSkillDoc)(raw);
            }
            catch {
                // try next
            }
    ...
    resolveSkillFilePath(skillId) {
        const userPath = path.join(this.userSkillsDir(), skillId, 'SKILL.md');
        if (fs.existsSync(userPath))
            return userPath;
        const dir = this.resolveSkillTemplatesDir();
        if (dir && fs.existsSync(path.join(dir, skillId, 'SKILL.md')))
            return path.join(dir, skillId, 'SKILL.md');
        return null;
    }
    ...
    async loadSkills() {
        ...
        try {
            ...
            if (start < 0 || end < 0)
                return [];
            const skills = JSON.parse(output.slice(start, end + 1));
    ...
    async runSkillAction(action, skillId, platform) {
        const snapshot = this.model.getSnapshot();
        const cwd = snapshot.workspacePath || undefined;
        const parts = ['deuk-agent-flow', 'skill', action, '--skill', (0, agentFlowViewUtils_1.shellArg)(skillId)];
        if (platform)
            parts.push('--platform', (0, agentFlowViewUtils_1.shellArg)(platform));
     ...
     * 그 외 액션은 그대로 위임.
     */
    async runSkillActionGuarded(action, skillId, platform) {
        if (action === 'expose' && platform) {
            ...
            if (category) {
                const conflicts = skills.filter((skill) => skill.id !== skillId &&
                    skill.category?.trim() === category &&
                    skill.exposed.includes(platform));
                if (conflicts.length > 0) {
                    const names = conflicts.map((skill) => skill.id).join(', ');
                    const proceed = await vscode.window.showWarningMessage(`'${platform}'에 이미 같은 종류(${category}) 스킬이 노출돼 있어요: ${names}. ${skillId}도 함께 노출할까요?`, { modal: true }, '노출');
                    if (proceed !== '노출') {
                        return;
                    }
                }
    ...
    async editSkill(skillId) {
        ...
        if (!fs.existsSync(userPath)) {
            ...
            const command = `deuk-agent-flow skill fork --skill ${(0, agentFlowViewUtils_1.shellArg)(skillId)} --non-interactive`;
     ...
     */
    async broadcastSkills(force = false) {
        if (this.skillsLoaded && !force) {
            if (this.activeTab === 'skill')
                this.postActiveTab('skill');
            return;
        }
    ...
    createCliCommand(kind, snapshot) {
    ...
    createTicketLifecycleCommand(action, ticketId) {
        return `deuk-agent-flow ticket ${action} --topic ${(0, agentFlowViewUtils_1.shellArg)(ticketId)} --non-interactive --compact`;
    }
    ...
    async setGlobalSettings(settings) {
        ...
        if (settings.ticketLanguage) {
            commands.push(`deuk-agent-flow config set docsLanguage ${(0, agentFlowViewUtils_1.shellArg)(settings.ticketLanguage)}`);
        }
        ...
        if (typeof settings.mcpAddress === 'string') {
            const mcpAddress = settings.mcpAddress.trim() || agentFlowModel_1.DEFAULT_DEUK_AI_CONTEXT_MCP_ADDRESS;
            commands.push(`deuk-agent-flow config set mcpAddress ${(0, agentFlowViewUtils_1.shellArg)(mcpAddress)}`);
        }
    ...
    async deliverToTarget(target, text) {
    ...
    async focusAndPaste(commands, text) {
        ...
        for (const command of commands) {
            ...
            await (0, agentFlowViewUtils_1.delay)(120);
        }
        if (!usedCommand) {
            return false;
        }
    ...
    buildState() {
        ...
        return {
            ...snapshot,
            workspaceBadge: snapshot.workspaceName ? `${snapshot.workspaceName} · ${(0, i18n_1.tf)('ticketsCountBadge', snapshot.tickets.length, snapshot.totalTicketCount)}` : (0, i18n_1.t)('noWorkspace'),
            targetLabel: (0, agentFlowModel_1.describeTarget)(snapshot.target),
            isMultiSelected: selectedTickets.length > 1,
            previewTicketActions: promptTicket ? (0, agentFlowModel_1.ticketLifecycleActions)(promptTicket) : [],
            previewTicketProgress: promptTicket ? (0, agentFlowModel_1.describeTicketProgress)(promptTicket) : 'no ticket',
    ...
    renderHtml(webview, mode) {
        ...
        const uiLang = (0, i18n_1.detectUiLang)();
        return `<!DOCTYPE html>
<html lang="${uiLang}">
<head>
  <meta charset="UTF-8" />
    ...
    <section class="grid">
      <article class="card ticket">
        <div class="row">
          <div class="brand">
            ...
            <div>
              ...
              <div class="version-line">VSIX <strong>${(0, agentFlowViewUtils_1.escapeHtml)(version)}</strong></div>
            </div>
          </div>
          <div class="button-row">
            <button class="secondary settings-button" data-action="toggle-settings" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('settings'))}">⚙</button>
            <button class="secondary" data-action="open-readme" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('help'))}">?</button>
          </div>
        </div>
        <div class="tab-bar">
          <button class="tab-button" data-tab-button="workspace">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('tabWorkspace'))}</button>
          <button class="tab-button" data-tab-button="skill">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('tabSkill'))}</button>
        </div>
        <div class="tab-panel" data-tab="workspace">
        ${state.workspaceLoading ? `
        <div class="workspace-loading-bar">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('loading'))}</div>
        <div class="ticket-list"><div class="tickets-empty">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('ticketListLoading'))}</div></div>
        ` : `
        <div class="workspace-row">
          <div class="workspace-toolbar">
            <select id="workspaceSelect" ${state.command.status === 'running' ? 'disabled' : ''}>
              ${state.workspaces
            .map((workspace) => `
                <option value="${(0, agentFlowViewUtils_1.escapeHtml)(workspace.path)}" ${workspace.path === state.workspacePath ? 'selected' : ''}>
                  ${(0, agentFlowViewUtils_1.escapeHtml)(`${workspace.label} · ${workspace.activeTicketCount}/${workspace.ticketCount} tickets`)}
                </option>`)
            .join('')}
            </select>
          </div>
          <button class="secondary fixed-refresh" data-action="refresh">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('refresh'))}</button>
        </div>
        ${(() => {
            const hasOutput = state.command.output && state.command.output.trim().length > 0;
            const flowLine = (0, agentFlowViewUtils_1.buildActiveFlowLineHtml)(state.workspaceName, state.activeTicket);
            // #650: 활성 티켓이 있거나 명령 출력이 있으면 카드를 항상 보여준다.
            // flowLine(📦 워크스페이스 · 단계 · #티켓)을 상단에 두어 터미널·채팅과 표현을 통일.
            if (!flowLine && !hasOutput)
                return '';
            return `
        <article class="card session">
          ${flowLine}
          <div class="meta" style="margin-top: 6px;">
            <span class="pill ${state.command.status === 'running' ? 'warn' : state.command.status === 'error' ? 'bad' : state.command.status === 'success' ? 'ok' : ''}">${(0, agentFlowViewUtils_1.escapeHtml)(state.command.status.toUpperCase())}</span>
            <span class="pill">${(0, agentFlowViewUtils_1.escapeHtml)(state.command.label || state.lastAction)}</span>
          </div>
          ${hasOutput ? `
          <div class="summary" style="margin-top: 8px;">
            <pre class="cmd-output" style="white-space: pre-wrap; margin: 0; font: inherit;">${(0, agentFlowViewUtils_1.renderCommandOutputHtml)(state.command.output)}</pre>
          </div>` : ''}
        </article>`;
        })()}
        <div class="ticket-toolbar">
          <span class="filter-control">
            ...
            <select id="statusFilter" class="${state.statusFilter !== 'all' ? 'lit-control' : ''}" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('filter'))}">
            <option value="open" ${state.statusFilter === 'open' ? 'selected' : ''}>${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('filterOpen'))}</option>
            <option value="closed" ${state.statusFilter === 'closed' ? 'selected' : ''}>${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('filterClosed'))}</option>
            <option value="all" ${state.statusFilter === 'all' ? 'selected' : ''}>${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('filterAll'))}</option>
            </select>
          </span>
          <button class="secondary ${state.selectedTicketIds.length > 0 ? 'lit-control' : ''}" data-action="copy-handoff" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('handoffTitle'))}">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('handoff'))}</button>
          <select id="ticketLifecycleAction" class="${state.selectedTicketIds.length > 0 || state.previewTicket ? 'lit-control' : ''}" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('lifecycleTitle'))}">
            <option value="">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('lifecyclePlaceholder'))}</option>
            ${state.previewTicketActions.map((action) => `<option value="${(0, agentFlowViewUtils_1.escapeHtml)(action)}">${(0, agentFlowViewUtils_1.escapeHtml)(action)}</option>`).join('')}
          </select>
          <span class="chip active-chip">${(0, agentFlowViewUtils_1.escapeHtml)(state.activeTicket?.id ?? 'none')}</span>
          <span class="selection-summary">
            <span class="chip selection-chip ${state.selectedTicketIds.length > 0 ? 'active' : ''}">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.tf)('selectionSelected', state.selectedTicketIds.length))}</span>
            <span class="chip selection-chip">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.tf)('selectionTickets', state.tickets.length, state.totalTicketCount))}</span>
          </span>
        </div>
        <div class="ticket-list">
          ${state.tickets.length === 0 ? `<div class="tickets-empty tickets-empty-cta">
            <div class="tickets-empty-title">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('ticketsEmptyTitle'))}</div>
            <div class="tickets-empty-hint">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('ticketsEmptyHint'))}</div>
            <button class="primary create-ticket-cta" data-action="create-ticket" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('createTicketTitle'))}">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('createTicketCta'))}</button>
          </div>` : state.tickets.map((ticket) => (0, agentFlowViewUtils_1.renderTicketItem)(ticket, state.activeTicket?.id, state.previewTicket?.id, state.selectedTicketIds)).join('')}
        </div>
        ${state.previewTicket ? (0, agentFlowViewUtils_1.renderPreviewPanel)(state.previewTicket, state.activeTicket?.id) : ''}
        `}
        </div>
        <div class="tab-panel" data-tab="skill">
      ${state.skillsLoading ? `<div class="skills-loading-bar">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('loading'))}</div>` : ''}
      <article class="card skills">
        <div class="row">
          ...
          <button class="secondary" data-action="skills-refresh" title="${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('skillsRefresh'))}">↻</button>
        </div>
        <div class="skills-hint">설치(+) 후 각 에이전트에 노출(ON/OFF). 스킬명 클릭=미리보기, edit=내 스킬로 편집.</div>
        <div class="skills-scroll">
          ${(() => {
            if (state.skillsLoading)
                return `<div class="skills-empty">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('skillsLoading'))}</div>`;
            if (state.skills.length === 0)
                return `<div class="skills-empty">${(0, agentFlowViewUtils_1.escapeHtml)((0, i18n_1.t)('skillsEmpty'))}</div>`;
            const CATEGORY_LIMITS = { persona: 1, coding: 3, data: 2 };
            ...
            return Object.entries(groups).map(([cat, skills]) => {
              ...
              <details class="skill-category-node" open>
                <summary class="skill-category-summary">
                  📁 ${(0, agentFlowViewUtils_1.escapeHtml)(cat)}${limitText}
                </summary>
                <table class="skills-table">
                  ...
                  <tbody>
                    ...
                    const cell = (platform) => {
                        ...
                        return `<td><button class="skill-toggle ${on ? 'on' : ''}" ${disabled} data-skill="${(0, agentFlowViewUtils_1.escapeHtml)(skill.id)}" data-platform="${platform}" data-on="${on ? '1' : '0'}" title="${on ? '노출 해제' : '노출'}">${on ? 'ON' : 'OFF'}</button></td>`;
                    };
                    const isPreview = skill.id === state.previewSkillId;
                    return `<tr class="${isPreview ? 'skill-row-active' : ''}">
                        <td class="skill-name skill-preview-trigger" data-skill="${(0, agentFlowViewUtils_1.escapeHtml)(skill.id)}" title="스킬 내용 미리보기">${(0, agentFlowViewUtils_1.escapeHtml)(skill.id)}</td>
                        <td><button class="skill-install ${skill.installed ? 'on' : ''}" data-skill="${(0, agentFlowViewUtils_1.escapeHtml)(skill.id)}" data-installed="${skill.installed ? '1' : '0'}" title="${skill.installed ? '설치 제거' : '설치'}">${skill.installed ? '✓' : '+'}</button></td>
        ...
        ${(0, agentFlowViewUtils_1.renderSkillPreview)(state.skills.find((skill) => skill.id === state.previewSkillId) ?? null)}
      </article>
        </div>
    </section>
    ${(0, agentFlowViewUtils_1.renderGlobalConfigPanel)(state.globalConfig)}
  </div>

  ${(0, agentFlowViewScript_1.buildWebviewScript)(nonce, encodedState, agentFlowModel_1.DEFAULT_DEUK_AI_CONTEXT_MCP_ADDRESS)}
</body>
</html>`;
    }
}
exports.AgentFlowCommandCenter = AgentFlowCommandCenter;
AgentFlowCommandCenter.viewType = VIEW_TYPE;
//# sourceMappingURL=agentFlowView.js.map