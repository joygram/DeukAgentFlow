...
import { AGENT_TOOLS, WORKFLOW_MODE_EXECUTE, WORKSPACE_KINDS, makePath, normalizeWorkflowMode, resolveDocsLanguage } from "./cli-utils.js";
...
export async function runInteractive(opts) {
    ...
    try {
        ...
        if (opts.remoteSync)
            console.log(`  ${cliText("prompts.init.summary.syncUrl", { locale, vars: { value: opts.pipelineUrl } })}\n`);
    }
    finally {
        rl.close();
    }
