...
import { DEUK_ROOT_DIR, TICKET_SUBDIR, makePath, toFileUri } from "./cli-utils.js";
...
export function normalizeMarkdownFile(absPath) {
    if (!statExists(absPath) || !isMarkdownFile(absPath))
        return false;
...
function lintFile(absPath, repoRoot) {
    ...
    while ((match = linkPattern.exec(content)) !== null) {
        ...
        if (!pathOnly)
            continue;
        if (isImplicitUpwardEvidenceLink(pathOnly))
            continue;
...
export function lintMarkdownPaths(paths, cwd = process.cwd()) {
    ...
    const targets = Array.from(new Set((Array.isArray(paths) ? paths : [])
...
//# sourceMappingURL=lint-md.js.map