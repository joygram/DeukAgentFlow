...
import { DEUK_ROOT_DIR, makePath, resolvePackageRoot, toPosixPath } from "./cli-utils.js";
...
function ticketWorkflowStateNameFromPhase(phase) {
    ...
    if (normalizedPhase >= 4)
        return "phase4";
    return `phase${normalizedPhase}`;
}
function ticketWorkflowStateNameFromMeta(meta = {}, fallbackStatus = "open") {
...
export function resolveTicketWorkflowTransition(meta = {}, opts = {}) {
...
export function resolveTicketWorkflowRuntimeState(meta = {}, fallbackStatus = "open") {
...
export function assertTicketWorkflowAction(meta = {}, opts = {}) {
    ...
    const discardHint = canDiscard
        ...
        : "";
    throw new Error(`[TICKET WORKFLOW BLOCKED] ${reasons.join(", ")}\n` +
        `Current state: ${name} | Allowed actions: ${allowedActions} | Valid transitions: ${allowedTransitions}${discardHint}`);
}
export function getTicketWorkflowTransitionReasons(meta = {}, opts = {}) {
...
export function assertTicketWorkflowTransition(meta = {}, opts = {}) {
    ...
    if (result.reasons.length > 0) {
        ...
        throw new Error(`[TICKET WORKFLOW BLOCKED] ${result.transition.from} -> ${result.transition.to}: ${result.reasons.join(", ")}\n` +
            `Valid transitions from ${result.transition.from}: ${validTransitions}`);
    }
    return result.transition;
}
function readProjectPolicyText(cwd, policyRole, language, replacements = {}) {
    if (!cwd || !policyRole)
        return "";
...
function readLocalizedPackagePrompt(rootName, promptName, language, replacements = {}, missingLabel = "ticket workflow recipe") {
    if (!promptName)
        return "";
...
function buildTicketStateRecipe(kind, language, replacements = {}, context = {}) {
    const workflowState = resolveTicketWorkflowState(kind);
    const statePrompt = readLocalizedPackagePrompt("state", workflowState.prompt, language, replacements, "ticket workflow state recipe");
    const projectPolicy = readProjectPolicyText(context.cwd, context.policyRole || workflowState.policyRole, language, replacements);
    const stateTemplate = readLocalizedPackagePrompt("state-template", context.template || workflowState.template, language, replacements, "ticket workflow state template");
...
function renderTicketWorkflowRuntimeContext(kind, workflowState, runtimeContext = null) {
    ...
    const runtimeLines = [
        ...
        `- Required evidence: ${Object.entries(requiredEvidence).map(([target, evidence]) => `${target}=[${evidence.join(", ")}]`).join("; ") || "none"}`,
        `- Current blockers: ${(runtimeContext.reasons || []).length ? runtimeContext.reasons.join(", ") : "none"}`
    ];
    if (runtimeContext.nextAction)
        runtimeLines.push(`- Next action: ${runtimeContext.nextAction}`);
    return runtimeLines.join("\n");
}
function normalizeTicketWorkflowReplacements(replacements = {}) {
    return Object.fromEntries(Object.entries(replacements).map(([key, value]) => [key, value == null ? "" : value]));
}
export function buildTicketDocMetaSurface(kind, opts = {}) {
    const language = resolveGuideLanguage(opts.docsLanguage, opts.resolveLanguage || ((value) => value), opts.normalizeDocsLanguage || ((value) => value));
...
export function renderTicketDocMetaSurface(kind, opts = {}) {
...
export function buildTicketFlowSurface(kind, opts = {}) {
    const docmeta = buildTicketDocMetaSurface(kind, opts);
...
export function renderTicketFlowSurface(kind, opts = {}) {
...
export function sanitizeTicketWorkspaceLabel(workspaceLabel) {
    ...
    if (!normalized)
        return "";
    if (/^(?:workspace|none|null|undefined)$/i.test(normalized))
        return "";
    return normalized;
}
...
export function getTicketGuide(kind, docsLanguage = "ko", resolveLanguage = (value) => value, normalizeDocsLanguage = (value) => value, context = {}) {
...
export function deriveTicketWorkflowStatus(meta = {}, fallbackStatus = "open") {
    ...
    if (phase >= 2)
        return rawStatus === "active" ? "open" : rawStatus;
...
export function inferTicketWorkflowStatus(meta = {}, isArchived = false) {
    if (isArchived)
        return "archived";
    return deriveTicketWorkflowStatus(meta, "open");
}
//# sourceMappingURL=ticket-workflow.js.map