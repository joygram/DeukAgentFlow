...
import { makePath } from "./cli-utils.js";
...
function pack(cwd, expected) {
    ...
    if (!existsSync(tarball))
        throw new Error(`packed tarball missing: ${tarball}`);
    const fileSet = new Set((row.files || []).map(file => file.path));
    for (const path of expected.files) {
        if (!fileSet.has(path))
            throw new Error(`${expected.name} tarball missing required file: ${path}`);
    }
...
function assertGlobalCommand(binDir, name) {
    ...
    if (!existsSync(script))
        throw new Error(`global command shim missing: ${script}`);
    if (process.platform !== "win32") {
        const mode = statSync(script).mode;
        if ((mode & 0o111) === 0)
            throw new Error(`global command is not executable: ${script}`);
    }
}
try {
...
//# sourceMappingURL=smoke-npm-local.js.map