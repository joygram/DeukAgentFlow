...
import { DEUK_ROOT_DIR, computeTicketPath, detectConsumerTicketDir, normalizeTicketGroup, parseFrontMatter, stringifyFrontMatter, writeFileLF, makePath } from "./cli-utils.js";
...
export function resolveTicketPath(cwd, relPath) {
    ...
    if (rel.startsWith(legacyPrefix))
        rel = rel.slice(legacyPrefix.length);
...
export function resolveTicketEntryOrComputedPath(cwd, entry) {
    ...
    if (String(entry?.status || "").toLowerCase() === "archived" && archivedYearMonth && archivedDay) {
        const dayPath = makePath(ticketDir, "archive", group, archivedYearMonth, archivedDay, fileName);
...
export function renderTicketDocument({ meta = {}, content = "", docmeta = null } = {}) {
...
//# sourceMappingURL=cli-ticket-document.js.map