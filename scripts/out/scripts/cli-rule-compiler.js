...
import { DEUK_ROOT_DIR, makePath, parseFrontMatter } from "./cli-utils.js";
...
export function compileDynamicRules(cwd, bundleRoot, targetFileName) {
    ...
    if (existsSync(bundleRulesDir)) {
        readdirSync(bundleRulesDir).forEach(f => {
            if (f.endsWith(".md"))
                allRuleFiles.push(makePath(bundleRulesDir, f));
        });
    }
    ...
    if (existsSync(localRulesDir)) {
        readdirSync(localRulesDir).forEach(f => {
            if (f.endsWith(".md"))
                allRuleFiles.push(makePath(localRulesDir, f));
        });
    }
    let compiledContent = "";
...
//# sourceMappingURL=cli-rule-compiler.js.map