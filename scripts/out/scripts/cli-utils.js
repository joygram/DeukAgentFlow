...
import { basename, dirname, relative, resolve, win32, posix } from "path";
...
function ticketHomeModule() {
    if (!_ticketHomeModule)
        throw new Error("ticket home module not registered");
...
export function writeFileLF(path, content, encoding = "utf8") {
...
export function deukRel(...segments) {
...
export function normalizeTicketGroup(rawGroup, fallback = "sub") {
    ...
    if (!value)
        return fallback;
    if (value.includes("/") || value.startsWith("TICKET-") && value.includes(".md"))
        return fallback;
    if (value.endsWith(".md"))
        return fallback;
    if (value.endsWith(".markdown"))
        return fallback;
    if (LEGACY_TICKET_GROUPS.has(value))
        return fallback;
    return value;
}
...
export function normalizeDocsLanguage(value) {
    ...
    if (!normalized || normalized === "auto")
        return "auto";
    if (normalized.startsWith("ko") || normalized === "kr" || normalized === "korean")
        return "ko";
...
export function inferDocsLanguageFromEnv(env = process.env) {
    ...
    if (posix)
        return posix.includes("ko") ? "ko" : "en";
...
export function resolveDocsLanguage(value, env = process.env) {
    ...
    if (normalized === "auto")
        return inferDocsLanguageFromEnv(env);
    return normalized;
}
...
export function loadInitConfig(cwd, opts = {}) {
    const targets = [
    ...
    for (const target of targets) {
        ...
        try {
            ...
            if (j.version !== INIT_CONFIG_VERSION)
                continue;
            if (!j.docsLanguage)
                j.docsLanguage = "auto";
            ...
            if (j.scmIgnoreDeukAgent === undefined)
                j.scmIgnoreDeukAgent = !Boolean(j.shareTickets);
            j.configPath = target;
            j.configScope = "user";
            return j;
        }
            ...
            if (process.env.DEBUG)
                console.warn(`[DEBUG] Failed to parse config ${target}:`, err);
        }
    }
    return null;
}
...
export function resolveUserHome(opts = {}) {
...
let _packageRootCache = null;
export function resolvePackageRoot(opts = {}) {
    if (opts.packageRoot)
        return opts.packageRoot;
    if (!opts.noCache && _packageRootCache)
    ...
    while (true) {
        if (existsSync(makePath(dir, "package.json")))
            break;
        const parent = dirname(dir);
        if (parent === dir)
            break; // reached root without a marker — fall back to original dir
        dir = parent;
    }
    if (!opts.noCache)
...
export function getUserConfigDir(opts = {}) {
    const platform = opts.platform || process.platform;
...
export function getUserInitConfigPath(opts = {}) {
...
export function getUserSkillsDir(opts = {}) {
...
export function getUserSkillsConfigPath(opts = {}) {
...
export function migrateWorkspaceInitConfigToUserConfig(cwd, opts = {}) {
...
export function isWorkflowExecute(opts = {}, savedConfig = null) {
    return normalizeWorkflowMode(opts.workflowMode ??
        opts.workflow ??
        opts.approval ??
        opts.approvalState ??
        savedConfig?.workflowMode ??
        savedConfig?.approvalState) === WORKFLOW_MODE_EXECUTE;
}
/**
...
export function buildPromptWorkspaceAliases(workspace) {
    ...
    for (const registryAlias of workspace?.registryAliases || []) {
        ...
        if (trimmed && isSpecificPromptAlias(trimmed))
            promptAliases.add(trimmed);
    }
...
function directProjectRoot(startDir) {
    ...
    if (parent !== current && hasWorkspaceMarker(parent))
        return parent;
    return null;
}
export function normalizeRegistryPath(value) {
    ...
    if (wsl2)
        p = wsl2[1].toUpperCase() + ":" + (wsl2[2] || "\\").replace(/\//g, "\\");
    // WSL1 / Git Bash: /d/... ??D:\...
    const wsl1 = p.match(/^\/([a-zA-Z])(\/.*)?$/);
    if (wsl1)
        p = wsl1[1].toUpperCase() + ":" + (wsl1[2] || "\\").replace(/\//g, "\\");
...
export function getSiblingWorkspaceRegistryGroupPath(parentRoot, opts = {}) {
    const homeDir = resolveUserHome(opts);
    return makePath(getSiblingWorkspaceRegistryRoot(homeDir), hashWorkspacePath(parentRoot));
}
export function loadRegistrySiblingWorkspaceCandidates(startDir, opts = {}) {
    const current = resolve(startDir);
    ...
    try {
        for (const entry of readdirSync(registryDir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            if (!id || !workspacePath || !hasWorkspaceMarker(workspacePath))
                continue;
            ...
            if (isInsideHomeStore(workspacePath))
                continue;
...
export function loadAllRegistryWorkspaceCandidates(opts = {}) {
    const homeDir = resolveUserHome(opts);
        ...
        for (const ent of readdirSync(ticketsRoot, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            try {
                ...
                if (Array.isArray(meta?.aliases))
                    aliases = meta.aliases.map(a => String(a || "").trim()).filter(Boolean);
            }
            catch {
                continue;
            } // no workspace.json → not a candidate yet
            // Path must still exist with a workspace marker, and not be the home store.
            if (!workspacePath || !hasWorkspaceMarker(workspacePath))
                continue;
            if (isInsideHomeStore(workspacePath))
                continue;
            ...
            if (isHomeDirectChild(workspacePath, opts))
                continue;
            if (!name)
    ...
    const kept = all.filter(w => !paths.some(p => p !== w.path && (w.path === p + "/" || w.path.startsWith(p + "/") || w.path.startsWith(p + "\\"))));
...
export function loadWorkspaceCandidates(startDir, opts = {}) {
...
export function findPromptWorkspaceMatches(prompt, candidates) {
    ...
    if (!normalizedPrompt || !candidates?.workspaces?.length)
        return [];
    const matches = [];
    for (const workspace of candidates.workspaces) {
        ...
        if (bestAliasLength > 0)
            matches.push({ workspace, bestAliasLength });
    }
    if (matches.length === 0)
        return [];
...
export function resolveSessionId(explicitId) {
    if (explicitId)
        return String(explicitId).trim().slice(0, 36);
...
function cookieHomeDir(opts = {}) {
    return makePath([resolveUserHome(opts), DEUK_ROOT_DIR], opts);
}
export function writeCookieFile(workspaceId, ticketId, sessionId, opts = {}) {
...
export function readCookieFile(sessionId, opts = {}) {
    const dir = cookieHomeDir(opts);
    if (!existsSync(dir))
        return null;
    const sid = sanitizeCookieSegment(sessionId);
    ...
    for (const f of files) {
        ...
        try {
            ...
            if (Date.now() - mtime > COOKIE_TTL_MS) {
                try {
                    unlinkSync(p);
                }
                catch { }
...
export function releaseCookieFile(sessionId, opts = {}) {
    const dir = cookieHomeDir(opts);
    if (!existsSync(dir))
        return;
    const sid = sanitizeCookieSegment(sessionId);
    ...
    for (const f of files) {
        try {
            unlinkSync(makePath([dir, f], opts));
        }
        catch { }
    }
}
export function listCookiesByTicket(workspaceId, ticketId, opts = {}) {
    const dir = cookieHomeDir(opts);
    if (!existsSync(dir))
        return [];
    const wp = sanitizeCookieSegment(workspaceId);
    ...
    return readdirSync(dir)
        ...
        try {
            ...
            return { sessionId: sid, path: p, mtime };
        }
        catch {
            return null;
        }
...
export function getWorkspaceCookiePath(_cwd, sessionId, opts = {}) {
...
export function readWorkspaceCookie(_cwd, sessionId, opts = {}) {
    ...
    const matched = candidates?.workspaces?.find(w => sanitizeCookieSegment(w.id) === cookie.workspaceId || sanitizeCookieSegment(w.path) === cookie.workspaceId);
    return matched ? { workspacePath: matched.path, workspaceId: matched.id } : null;
}
export function writeWorkspaceCookie(_cwd, sessionId, targetPath, opts = {}) {
    const sid = resolveSessionId(sessionId);
...
export function resolveWorkspaceTarget(startDir, query, opts = {}) {
    ...
    const matches = candidates.workspaces
        ...
        .sort((a, b) => a.id.localeCompare(b.id));
    if (matches.length === 0)
        return { candidates, target: null, reason: "no-match" };
    if (matches.length > 1) {
        ...
        const exact = matches.filter(workspace => (workspace.aliases || []).some(alias => String(alias).toLowerCase() === rawQuery.toLowerCase()));
        if (exact.length === 1) {
...
export function buildWorkspaceGuide(startDir, opts = {}) {
    ...
    if (!reason) {
        if (!candidates?.workspaces?.length)
            reason = "no sibling workspace candidates found";
        else
            reason = "no explicit workspace target detected";
    }
    return {
        hostWorkspace: {
...
export function requireNonEmptySlug(input, fieldName = "value") {
    ...
    throw new Error(`[VALIDATION FAILED] ${fieldName} must produce a non-empty ASCII slug. ` +
...
export function inferRefTitleAndTopic(opts) {
    ...
    try {
        ...
        if (process.env.DEBUG)
            console.warn(`[DEBUG] Failed to read ref ${refAbs}:`, err);
        return null;
    }
...
export function sanitizeFrontMatterMeta(meta = {}) {
    ...
    if (cleanMeta.project === 'global')
        delete cleanMeta.project;
    if (!cleanMeta.submodule)
        delete cleanMeta.submodule;
...
export function findWorkspaceRoot(startDir) {
    ...
    while (curr && curr !== dirname(curr)) {
        if (curr === home)
            return null;
        if (hasWorkspaceMarker(curr))
            return curr;
        if (hasLocalProjectBoundary(curr) && curr !== resolve(startDir))
            return null;
        curr = dirname(curr);
    }
    if (curr !== home && hasWorkspaceMarker(curr))
        return curr;
    return null;
}
...
export function detectConsumerTicketDir(startDir, opts = {}) {
    ...
    try {
        ...
        if (home.isHomeStoreRoot(root, opts))
            return null;
    }
...
function isInsideHomeStore(cwd, opts = {}) {
...
export function isHomeDirectChild(p, opts = {}) {
...
export function resolveRealWorkspaceRoot(cwd, opts = {}) {
    if (opts && opts.workspace)
        return normalizeRegistryPath(opts.workspace);
...
export function isInsideGitWorkTree(cwd) {
    ...
    while (curr && curr !== dirname(curr)) {
        if (isGitWorkTreeMarker(makePath(curr, ".git")))
            return true;
        curr = dirname(curr);
    }
...
export function discoverAllWorkspaces(baseCwd, ignoreDirs = DEFAULT_IGNORE_DIRS, out = new Set()) {
    ...
    try {
        ...
        for (const ent of entries) {
            if (!ent.isDirectory())
                continue;
            if (ignoreDirs.includes(ent.name) || ent.name.startsWith(".deuk-agent"))
                continue;
        ...
        if (process.env.DEBUG)
            console.warn(`[DEBUG] Failed to read directory ${baseCwd}:`, err);
    }
    return Array.from(out);
}
async function probeMcpUrl(url) {
    ...
    for (const method of methods) {
        ...
        try {
            ...
            if (process.env.DEBUG)
                console.warn(`[DEBUG] SSE ${method} ping failed for ${url}:`, err);
        }
        finally {
            clearTimeout(timeoutId);
        }
    ...
    for (const p of mcpPaths) {
        if (existsSync(p)) {
            try {
                ...
                if (process.env.DEBUG)
                    console.warn(`[DEBUG] Failed to parse MCP config ${p}: ${err.message}`);
                continue;
            }
        }
    }
...
export async function isMcpActive(cwd) {
    ...
    for (const status of statuses) {
        if (status.command)
            return true; // Stdio is managed by IDE
        if (status.url && await probeMcpUrl(status.url))
            return true;
    }
    return false;
}
