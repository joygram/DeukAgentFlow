...
import { parseFrontMatter, discoverAllWorkspaces, detectConsumerTicketDir, computeTicketPath, makePath } from "./cli-utils.js";
...
export function rebuildTicketIndexFromTopicFilesIfNeeded(cwd, opts = {}) {
    ...
    if (opts.rebuild)
        console.log(`[REBUILD] Ticket pointer synced; ${indexJson.entries.length} entries derived from .md.`);
    return indexJson;
}
...
export function updateTicketEntryStatus(cwd, opts = {}) {
    ...
    if (opts.latest) {
        ...
        foundIndex = indexJson.entries.findIndex(e => String(e.id || "").toLowerCase().includes(targetId));
    }
    if (foundIndex === -1) {
...
//# sourceMappingURL=cli-ticket-parser.js.map