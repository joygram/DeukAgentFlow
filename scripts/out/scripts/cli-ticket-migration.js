...
import { toRepoRelativePath, TICKET_SUBDIR, TICKET_LIST_FILENAME, parseFrontMatter, findFileRecursively, detectConsumerTicketDir, makePath } from "./cli-utils.js";
...
export function extractSummary(meta, content) {
    ...
    const scopeKeys = Object.keys(sections).filter(k => k.includes("scope") || k.includes("target") || k.includes("background"));
    ...
    if (!contextLine) {
        const analysisKeys = Object.keys(sections).filter(k => k.includes("analysis") || k.includes("design") || k.includes("decisions"));
    ...
    const taskKeys = Object.keys(sections).filter(k => k.includes("task") || k.includes("phased") || k.includes("execution steps"));
    let taskItems = [];
...
export function ensureCautionBlock(content) {
    ...
    if (content.includes("## Target Module"))
        return null;
    ...
    if (after.includes("[CAUTION FOR AI AGENTS]"))
        return null;
    return before + "\n" + CAUTION_BLOCK + "\n" + after;
}
// ─── Main Migration ─────────────────────────────────────────────────────────
export function performUpgradeMigration(cwd, opts = {}) {
    ...
    for (const abs of files) {
        ...
        if (dirty) {
            if (opts.dryRun) {
                const changes = [];
                if (!parseFrontMatter(body).meta.summary && meta.summary)
                    changes.push("summary");
                if (cautionResult !== null)
                    changes.push("caution+targetModule");
...
export function normalizeTicketPaths(cwd, opts = {}) {
    ...
    if (modified) {
        ...
        if (!opts.silent)
            console.log(`[NORMALIZE] Corrected stale paths in ${basename(cwd)}/INDEX.json`);
    }
    return modified;
}
//# sourceMappingURL=cli-ticket-migration.js.map