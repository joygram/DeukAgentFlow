import { toSnakeCaseKey } from "./cli-utils.js";
...
const PHASE1_HEADING_ALIASES = new Map([
    ...
    ...REQUIRED_PHASE1_SECTIONS.map(section => [section.toLowerCase(), section])
]);
...
function hasSubstantiveSectionContent(text, scaffolds = []) {
    ...
    if (!src || hasPlaceholderTokens(src))
        return false;
...
export function getPhase1PlanBodyReasons(body, opts = {}) {
...
export function followUpDecisionMeansNoFollowUp(content) {
    ...
    if (!hasSubstantiveSectionContent(text, []))
        return false;
...
export function buildSlotStatusSummary(content, phase = 2) {
    ...
    for (const name of sections) {
        if (hasSubstantiveSectionContent(extractMarkdownSection(content, name), []))
            filled.push(name);
        else
            empty.push(name);
    }
...
//# sourceMappingURL=cli-ticket-evidence.js.map