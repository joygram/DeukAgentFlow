...
import { DEUK_ROOT_DIR, makePath } from "./cli-utils.js";
...
function resolveTaskGrade(input = "") {
    ...
    if (!VALID_TASK_GRADES.has(grade))
        return "";
    return grade;
}
...
async function runUsageSet(opts) {
    ...
    if (next.agentId)
        console.log(`agent: ${next.agentId}`);
    if (next.weeklyResetAt)
        console.log(`reset: weekly ${next.weeklyResetAt}`);
    if (next.fiveHourResetAt)
        console.log(`reset: 5h ${next.fiveHourResetAt}`);
}
async function runUsageStatus(opts) {
    ...
    if (state.weeklyResetAt)
        console.log(`reset: weekly ${state.weeklyResetAt}`);
    if (state.fiveHourResetAt)
        console.log(`reset: 5h ${state.fiveHourResetAt}`);
}
...
//# sourceMappingURL=cli-usage-commands.js.map